...
--Detection Lag Distribution (First Query)
SELECT 
    ROUND((JULIANDAY(m.detected_at) - JULIANDAY(p.prompt_timestamp)) * 24, 1) AS hours_to_detection
FROM prompts p
JOIN model_outputs m ON p.prompt_id = m.prompt_id

--Detection Lag Distribution (Updated Query)
SELECT 
    CASE 
